...
**Data model** (`database/migrations/20260515012938_init_tables.js`, reconciled with `SPEC.md`): `conferences → divisions → teams`, `seasons → weeks → games`, plus `users` and `picks` (all plural snake_case). There is no `outcome` table — game outcome lives on `games.result` and per-pick correctness on `picks.is_correct`.
